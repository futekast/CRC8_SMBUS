...
//------------------------------------------------------------------------------
// CRC-8/SMBus lookup table for polynomial 0x07.
//------------------------------------------------------------------------------

static const uint8_t crc8SmbusTable[256] = {
...
uint8_t initializeCrc8Smbus(void) {
    return _CRC8_SMBUS_INITIAL_VALUE();
}
...
uint8_t updateCrc8SmbusBitwise(uint8_t crcValue, uint8_t dataByte) {
    ...
    crcValue = (uint8_t)(crcValue ^ dataByte);

    for (bitIndex = 0u; bitIndex < 8u; bitIndex++) {
        if ((crcValue & 0x80u) != 0u) {
            crcValue = (uint8_t)((crcValue << 1u)
                                 ^ _CRC8_SMBUS_POLYNOMIAL());
        } else {
            crcValue = (uint8_t)(crcValue << 1u);
        }
    }
...
uint8_t updateCrc8SmbusTable(uint8_t crcValue, uint8_t dataByte) {
    return crc8SmbusTable[(uint8_t)(crcValue ^ dataByte)];
}
...
uint8_t calculateCrc8SmbusBitwise(const uint8_t *pData, size_t dataLength) {
    ...
    crcValue = initializeCrc8Smbus();

    if (pData != NULL) {
        for (dataIndex = 0u; dataIndex < dataLength; dataIndex++) {
            crcValue = updateCrc8SmbusBitwise(crcValue, pData[dataIndex]);
        }
    }

    crcValue = (uint8_t)(crcValue ^ _CRC8_SMBUS_FINAL_XOR_VALUE());
...
uint8_t calculateCrc8SmbusTable(const uint8_t *pData, size_t dataLength) {
    ...
    if (pData != NULL) {
        for (dataIndex = 0u; dataIndex < dataLength; dataIndex++) {
            crcValue = updateCrc8SmbusTable(crcValue, pData[dataIndex]);
        }
    }
